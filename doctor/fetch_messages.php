...
while($row = $result->fetch_assoc()){
    $hasMessages = true;
    // Check if message was sent by the current user (doctor)
    ...
    if (isset($row['sender_type']) && isset($row['receiver_type'])) {
        ...
        $isSent = ($row['sender_id'] == $sender && $row['sender_type'] == 'doctor');
    } else {
...
// Debug: Show if no messages found (can be removed later)
if (!$hasMessages && $sender > 0 && $receiver > 0) {
    // Check if there are any messages at all for debugging
    $debug_query = "SELECT COUNT(*) as count FROM messages WHERE (sender_id = ? AND receiver_id = ?) OR (sender_id = ? AND receiver_id = ?)";
    $debug_stmt = $database->prepare($debug_query);
    $debug_stmt->bind_param("iiii", $sender, $receiver, $receiver, $sender);
    $debug_stmt->execute();
    $debug_result = $debug_stmt->get_result();
    $debug_row = $debug_result->fetch_assoc();
    // No output for empty chat - let JavaScript handle it
}
?>
